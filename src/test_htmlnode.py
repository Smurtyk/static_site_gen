...
class TestHTMLNode(unittest.TestCase):
    ...
    def test_with_children(self):
        ...
        self.assertEqual(
            node.__repr__(),
            '<div href="https://www.google.com">filler text<p>first</p><p>seccond</p></div>'
        )

    def test_with_grandchildren(self):
        gchild = HTMLNode('b', 'grandchild')
        child = HTMLNode('p', 'child', [gchild])
        node = HTMLNode('div', None, [child])
        self.assertEqual(node.__repr__(), '<div><p>child<b>grandchild</b></p></div>')
